...
import { buildProbeLoginParams, extractAuthSummary } from './auth.mjs';

function readProbeStub(auth) {
  const raw = process.env.CODEX_ACCOUNT_PROBE_STUBS;
  if (!raw) {
    return null;
  }

  const summary = extractAuthSummary(auth);
  const label = summary.email?.split('@')[0] ?? summary.chatgptAccountId ?? null;
  if (!label) {
    return null;
  }

  const stubs = JSON.parse(raw);
  return stubs[label] ?? null;
}

async function probeAuthRateLimitsFromStub(auth) {
  const stub = readProbeStub(auth);
  if (!stub) {
    return null;
  }

  if (stub.delayMs) {
    await delay(stub.delayMs);
  }
  ...
  return {
    ...
    account: {
      type: 'chatgpt',
      email: extractAuthSummary(auth).email ?? null,
      planType: extractAuthSummary(auth).planType ?? null,
    },
    rateLimits: stub.rateLimits ?? null,
    rateLimitsByLimitId: stub.rateLimits ? { codex: stub.rateLimits } : null,
    error: stub.success === false ? stub.error ?? 'stubbed probe failure' : undefined,
  };
}
...
function formatProbeError(error, stderr) {
  const upstreamPanic = stderr.match(/Attempted to create a NULL object\./);
  const detail = stderr.trim();

  if (upstreamPanic) {
    return 'Live probing is unavailable because the local codex binary panicked before app-server startup (system-configuration NULL object on macOS).' +
      (detail ? ` | ${detail}` : '');
  }
...
export async function probeAuthRateLimits(auth, { codexBin = 'codex', cwd, timeoutMs = 8_000 } = {}) {
  const stubbed = await probeAuthRateLimitsFromStub(auth);
  if (stubbed) {
    return stubbed;
  }
  ...
  try {
    ...
    return {
      success: false,
      probedAt: new Date().toISOString(),
      error: formatProbeError(error, stderr),
    };
  } finally {
